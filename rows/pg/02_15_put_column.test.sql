BEGIN;
    ...
    SELECT col_type_is( current_schema(), 'test', 'nb', 'pg_catalog', 'integer', 'check-type' );

    SELECT frmdb_put_column('test', 'nb', 'character varying(20)', null, null);
    SELECT has_column( 'test', 'nb' );
    SELECT col_type_is( current_schema(), 'test', 'nb', 'pg_catalog', 'character varying(20)', 'check-type' );

    SELECT frmdb_put_column('test', 'nb', 'numeric(5,2)', null, null);
    SELECT has_column( 'test', 'nb' );
    SELECT col_type_is( current_schema(), 'test', 'nb', 'pg_catalog', 'numeric(5,2)', 'check-type' );

    SELECT frmdb_put_column('test', 'enumcol', 'text', 'enumcol ~* ''A|B''', null);
    SELECT has_column( 'test', 'enumcol' );
    SELECT col_type_is( current_schema(), 'test', 'enumcol', 'pg_catalog', 'text', 'check-type' );
