...
import {
  ...
  Record, CreateParams, CreateResult, ValidUntil
} from 'ra-core';

export interface CreateManyResult<RecordType = Record> {
  data: RecordType[];
  validUntil?: ValidUntil;
}
export interface FrmdbDataProvider extends DataProvider {
  createMany: <RecordType extends Record = Record>(resource: string, params: CreateParams) => Promise<CreateManyResult<RecordType>>;
}

/**
 * @fork https://github.com/raphiniert-com/ra-data-postgrest 
 ...
 * export default App;
 */
function parseFilters(filter: any, defaultListOp: string) {
  let result: any = {};
  Object.keys(filter).forEach(function (key) {
    ...
    values.forEach(value => {
      let op = operation.includes('like') && value.indexOf('*') < 0 ? `${operation}.*${value}*` : `${operation}.${value}`;
...
const getKeyData = (primaryKey: PrimaryKey, data: { [x: string]: any }): object => {
  if (isCompoundKey(primaryKey)) {
...
export default (apiUrl: string, httpClient = fetchUtils.fetchJson, defaultListOp = 'eq',
  primaryKeys: Map<string, PrimaryKey> = defaultPrimaryKeys): DataProvider => ({
    ...
    getMany: (resource, params) => {
      ...
      return httpClient(url).then(({ json }) => ({ data: json.map((data: any) => dataWithId(data, primaryKey)) }));
    },
    ...
    createMany: (resource, params) => {
      ...
      return httpClient(url, {
        method: 'POST',
        headers: new Headers({
          'Accept': 'application/json',//needed for create with multiple objects
          //TODO: sometimes upsert does not work, after postgrest restart seems to start working...
    ...
    deleteMany: (resource, params) => {
      ...
      }).then(({ json }) => ({ data: json.map((data: any) => encodeId(data, primaryKey)) }));
    },
  });
